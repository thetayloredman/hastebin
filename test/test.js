var assert = require("assert");
var getStdin = require("get-stdin");
var got = require("got");
var jsapi = require("../src/hastebin");

var testData = "test";

function assertHastebinUrlMatchesTestData(input) {
    var hastebinUrl = input.trim();
    assert(hastebinUrl.match(/^https:\/\/paste\.0xlogn\.dev\/.*/) != null, "output must be a LogN hastebin url");
    var key = hastebinUrl.split("/").slice(-1)[0];
    return got("https://paste.0xlogn.dev/raw/" + key).then(function (result) {
        assert.equal(result.body.trim(), testData, "hastebin contents must equal test data");
    });
}

console.log("testing stdin...");
getStdin()
    .then(assertHastebinUrlMatchesTestData)
    .then(function () {
        console.log("testing jsapi non-raw...");
        jsapi
            .createPaste(testData)
            .then(assertHastebinUrlMatchesTestData)
            .then(function () {
                console.log("testing jsapi raw...");
                jsapi.createPaste(testData, { raw: true }).then(assertHastebinUrlMatchesTestData);
            });
    });
